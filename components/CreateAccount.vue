        ...
        <UCard>
            ...
            <UContainer v-if="identityObjectProxy">
                ...
                <UButton size="xl" :disabled="!isSeed" label="Create Account With Identity"
                    @click="createAccountWithIdentity" />
            </UContainer>
...
const createAccountWithIdentity = async () => {
    const global = await getCryptographicParameters();
    const identityIndex = 0
    const credNumber = 0
    const AccountSeed =seed.value // generateMnemonic(wordlist, 256);
    ...
    const signingKey = getAccountSigningKey(AccountSeed, credentialTransaction.unsignedCdi.ipIdentity);
