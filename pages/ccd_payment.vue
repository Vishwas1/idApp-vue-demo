  ...
  <UContainer>
    <div class="flex flex-col gap-2">
      <UCard style="width: 100%">
        ...
        <UButton class="mt-4" @click="payCCD">Pay {{ amount }} CCD</UButton>
        <div>CCD TransactionHas: {{ pltTransactionHash }}</div>
...
async function generateChallenge() {
  ...
  const body = {
    ...
    contextDetails: {
      amount: "1",
      purpose: "Testing CCD payment flow",
      tokenSymbol: "ccd",
      tokenType: "ccd",
      decimal: 2,
      recipientWalletAddress:
        "4cannFKDPJQ65Qi6ADJ4tVqakmpdDb9jfwRLKWsRY3QqpNcbtX",
    },
  };
...
async function payCCD() {
  try {
    let provider = window.concordium;
    const signatureInput = challenge.value.signatureInput;
    console.log(signatureInput);
    const { amount, toAddress, memo } = signatureInput.payload;
    const encodedMemo = Cbor.encode(memo);
    const memoPayload = new DataBlob(encodedMemo.bytes);
    const body = {
      amount: CcdAmount.fromMicroCcd(amount),
      toAddress: AccountAddress.fromBase58(toAddress),
      memo: memoPayload,
    };
    console.log(signatureInput.payload, "payload");
    console.log(body);
...
async function verify() {
  ...
  try {
    const resp = await fetch(
      `https://ai-plugin.nanocorp.io/api/v1/verify/tx-hash`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
